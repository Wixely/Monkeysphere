...
2026-09-07 M2 creation update: contract 1.10 implements create_domain with a client-generated new UUID, domains.manage, bounded registry reservations and retry/startup recovery. Browser creation uses the same publication path. Recovery after publication failure and backup/restore with pending intent are locally verified. Earlier creation design/foundation notes above describe now-completed work. Remaining M2 work: structured filter/sort search and the full two-domain MCP-only management scenario. Owner: Agent; next action: implement structured search; review: 2026-09-14.


2026-09-07 M2 search/workflow update: contract 1.11 implements query_records under records.read with AND filters, field/name sorting, strict limits, domain-bound references and structured errors. Shared search now reads totals/results in one transaction, rejects non-finite numeric bounds and handles equal exact-date boundaries correctly. The MCP-only integration scenario creates two domains, completes blank setup, creates schema and two typed/aliased records per domain, links, edits, filters/sorts and deletes selected dependent data after review. It proves isolation and replays every write under a rotated read-only credential to verify denial. The M2 application scenario is locally verified; live browser/client evidence and shared M0-M1 gates remain open. Next action: Agent implements M3 bounded upload sessions and durable contact previews. Review: 2026-09-14.


2026-09-07 M3 storage progress: implemented and locally tested the separate DnaX upload staging database, bounded session/chunk persistence, atomic offsets, restart replay, credential/domain ownership, quotas, checksum verification, expiry/cancellation cleanup and backup/restore exclusion. All 220 local tests pass. This is an internal foundation; no upload tool or contacts.import grant is exposed. Next action: Agent connects authenticated upload adapters, effective transport limits, redacted audit and vCard content validation, then durable preview/apply. Live MCPHub transfer and the rest of M3-M7 remain outstanding. Review: 2026-09-14.


2026-09-07 M3 validation progress: streaming Core vCard parsing and staged-contact validation are implemented, sharing browser parsing/fingerprint rules. Tests cover arbitrary UTF-8 splits, surrogate boundaries, opaque/folded properties, exact byte/card/property limits, cancellation, staged read-lease disposal, malformed content and absence of record mutations. All 228 local tests pass. Next action: Agent adds redacted upload audit and authenticated contacts.import lifecycle tools with effective transport bounds, then durable preview/apply. Upload tools and the rest of M3-M7 remain incomplete; live-client verification is still outstanding. Review: 2026-09-14.


2026-09-07 M3 upload delivery: contract 1.12 implements the five upload lifecycle tools with contacts.import, effective transport/file bounds, streaming content validation and staging schema 2 audit. Real MCP tests cover a full 5 MiB file, sequential/repeated chunks, malformed content, scope and domain denial, expiry, generation isolation and revocation midway. Audit insertion failure rolls back staging mutations. All 239 tests pass locally. Next action: Agent implements durable credential/domain-owned contact previews, paged inspection and revision-checked atomic apply, then exports/images. M3 remains incomplete and live MCPHub transfer plus broader M4-M7 work remain open. Review: 2026-09-14.

2026-09-07 contact preview foundation: shared browser imports now capture a persistent domain revision and reject stale previews atomically, including changes after preparation. Application migration 27 covers record/schema/provenance changes and transactional rollback. Streaming browser preview and shared preparation/application entry points are implemented. Durable MCP preview persistence, paged inspection and replayable apply remain unfinished; upload completion still imports no records. Owner: Agent; next action: implement bounded credential/domain-owned preview artifacts and atomic import receipts, followed by export/images and the remaining management roadmap. Review: 2026-09-14.
2026-09-07 durable preview storage: staging migration 3 persists credential/domain/source-owned preview headers and ordered complete contact evidence. Same-key replay preserves the reviewed revision across restart; preview payloads expire after 15 minutes or earlier source expiry/cancellation, with bounded quotas, cleanup and retry tombstones. Transactional audit, concurrency, fidelity, all quota dimensions, oversized evidence and backup/restore invalidation have dedicated tests. No preview MCP tool is exposed yet. Owner: Agent; next action: response-size-bounded inspection projections and atomic application-database import receipts/outcomes, then export/images and M4-M7. Review: 2026-09-14.
2026-09-07 preview tool delivery: contract 1.13 exposes preview_contact_import, get_contact_import_preview and read_contact_import_evidence under contacts.import. Clients can page summaries and reconstruct full mappings/opaque properties/duplicate evidence from bounded byte ranges. Advisory current-revision status, original-expiry replay, ownership checks and redacted inspection audit are implemented. Person-preset validation, a full 5 MiB preview, Unicode evidence assembly, scope denial, rotation and cancellation are tested through MCP. Owner: Agent; next action: atomic revision-checked apply with durable per-contact outcomes/receipts, then contact export/images and M4-M7. Live MCPHub verification remains outstanding. Review: 2026-09-14.

2026-09-07 contact apply delivery: contract 1.14 exposes apply_contact_import and get_contact_import_result under contacts.import. Application migration 28 stores mutation-atomic receipts, audit and paged outcomes for up to 1,000 contacts. Exact retries resolve before staging access; changed retries and second-key consumption fail, while stale previews roll back. Local Data/MCP tests cover all four actions, record revisions, opaque provenance, 1,000 skip outcomes, restart/cancellation replay, credential isolation and audit failure rollback. Owner: Agent; next action: bounded authenticated selected-contact export, followed by images and M4-M7. Live MCPHub verification remains outstanding. Review: 2026-09-14.
