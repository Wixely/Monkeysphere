...
- Authentication before location queries or retrieval. Coordinates, accuracy, and approximation radii are treated as sensitive record data and are returned only through the existing authenticated application and scoped remote-record boundaries. The map-pin editor uses vendored OpenLayers code with a local graticule and does not disclose coordinates to an external tile or geocoding provider.
