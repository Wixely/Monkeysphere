...
| Domains | Implemented `list_domains`, `create_domain`, `rename_domain` | Existing `records.read`; `domains.manage` | Deployment; create supplies a new UUID; rename targets an existing domain ID / M2 |
| Setup and presets | Implemented: `get_setup_state`, `list_presets`, `list_starter_packs`, `list_relationship_presets`, `list_installed_presets`, `install_preset`, `complete_setup`; revision-checked selection and acknowledged blank setup | `records.read` for reads; `structure.write` for installation/completion | Domain / M2 |
...
| Search | Implemented `query_records` for typed filters/sort and snapshot-consistent paging; legacy `search_records` preserved | `records.read` | Domain / M2 |
| Bulk records | `preview_record_batch`, `apply_record_batch`; explicit IDs, bounded one-domain transaction | `records.write` | Domain / M2 |
| Relationships | Existing record relationships; `list_relationship_types`, `create_relationship_type`, `create_relationship`, `delete_relationship`; directional endpoints and note | Implemented: `records.read` for reads, `relationships.write` for links, `structure.write` for type creation | Domain / M2 |
...
Domain rename foundation reviewed 2026-09-07: registry migration 2, shared browser revision checks and revision disclosure through both MCP domain read tools are implemented and locally tested. Contract 1.9 now implements revision-checked rename with registry receipts, atomic audit and restart replay. Contract 1.10 implements recoverable creation with bounded registry reservations and verified restart/backup recovery. Owner: Agent; next action: bounded contact upload and preview sessions; review: 2026-09-14.

Remaining work: finalize preview/upload quota bounds and live transfer verification, then finish B-F. Recommended next action: Agent implements bounded contact upload and durable preview sessions. Version 1.4 documents locally tested single/batch record writes, reviewed deletion, bounded previews, durable media cleanup and advertised limits. No additional integration installation or live configuration change is authorized by this plan.

2026-09-07 follow-up: contract 1.6 includes relationship/type revision metadata and scoped, replayable relationship creation/removal and type creation. Migration 25 protects shared browser mutation paths; mutation/receipt/audit atomicity and real MCP calls are locally tested. These basic relationship tools are Included. Owner: Agent. Contract 1.7 also implements custom type creation and field creation/attachment with persistent revisions and shared required-value validation. Contract 1.8 implements onboarding and preset discovery/installation with revision checks and durable receipts. Next action: bounded contact upload and durable preview sessions. Review: 2026-09-14.


2026-09-07 M3 option checkpoint: internal contact staging now has implemented quotas and restart-safe byte/offset transactions; see mcp-write-contract.md for bounds and retention. Begin/write/status/seal/cancel store operations are locally tested, including exclusion from backups and invalidation on restore. MCP upload tools remain planned until scope, audit, content-validation and transport-bound tests pass. Owner: Agent; next action: authenticated contact upload adapters and validation, followed by durable contact previews; review: 2026-09-14.


2026-09-07 content-validation update: internal contact completion can now seal integrity and stream the staged bytes through shared Core vCard validation. Semantic validation remains distinct from staging state and import. Owner: Agent; next action: redacted audit and authenticated upload lifecycle tools with transport limits, then durable contact previews/apply; review: 2026-09-14.


2026-09-07 upload lifecycle update: the five proposed upload tools are implemented under contacts.import in contract 1.12, including content validation, effective request-aware chunk limits and redacted mutation audit. No record import is implied by upload completion. Owner: Agent; next action: durable contact preview/inspection/apply, followed by export and image workflows; review: 2026-09-14.
