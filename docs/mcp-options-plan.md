...
Domain rename foundation reviewed 2026-09-07: registry migration 2, shared browser revision checks and revision disclosure through both MCP domain read tools are implemented and locally tested. Contract 1.9 now implements revision-checked rename with registry receipts, atomic audit and restart replay. Remote creation remains Deferred until creation recovery passes. Owner: Agent; next action: implement recoverable domain creation; review: 2026-09-14.

Remaining work: finalize preview/upload quota bounds and live transfer verification, then finish B-F. Recommended next action: Agent adds recoverable domain creation, then completes structured search before contact transfer. Version 1.4 documents locally tested single/batch record writes, reviewed deletion, bounded previews, durable media cleanup and advertised limits. No additional integration installation or live configuration change is authorized by this plan.

2026-09-07 follow-up: contract 1.6 includes relationship/type revision metadata and scoped, replayable relationship creation/removal and type creation. Migration 25 protects shared browser mutation paths; mutation/receipt/audit atomicity and real MCP calls are locally tested. These basic relationship tools are Included. Owner: Agent. Contract 1.7 also implements custom type creation and field creation/attachment with persistent revisions and shared required-value validation. Contract 1.8 implements onboarding and preset discovery/installation with revision checks and durable receipts. Next action: recoverable domain creation. Review: 2026-09-14.
