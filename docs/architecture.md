...
The multi-record map is an authenticated read model over structured location rows. Core validates latitude and longitude bounds, record-type and field filters, page number, and a maximum 500-row page size; the SQLite query supports viewports crossing the antimeridian and returns a deterministic page plus total count. The browser clusters only the returned page and exposes linked summaries for selected pins. It uses the same vendored, provider-free OpenLayers boundary as coordinate entry.
