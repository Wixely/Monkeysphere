...
Passing these checks does not prove the real systemd lifecycle. Before claiming systemd support, provision the documented `monkeysphere` account and paths on a disposable Linux host, install the unit, and verify enable, start, authenticated access, stop, restart, upgrade, and failure recovery.
