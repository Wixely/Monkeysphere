...
| M2 | Records, relationships, domain setup, and minimum structure creation | In progress; single/batch record writes and reviewed deletion pass real MCP tests; scoped relationship writes verified; minimum schema creation and onboarding verified; domain creation and search remain | Agent | 2026-09-21 |
...
Dates are review checkpoints, not delivery commitments. Recommended next action: Agent adds recoverable domain creation, then structured search while closing the remaining M0-M1 gates. Contact upload/preview/apply/export is the next user-facing delivery after basic editing and its transfer prerequisites.
