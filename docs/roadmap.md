...
| Framework-dependent Linux package on an independently installed ASP.NET Core runtime | Packaging and archive inspection pass; a self-contained publish ran on WSL2 | Execution of the distributed package against a separately installed runtime | Wixely / Agent | 2026-09-28 |
| systemd system-unit installation and boot enablement | `systemd-analyze verify` passed statically; a non-privileged transient user service passed lifecycle checks | Packaged unit installation, boot enablement, upgrade, and failure recovery | Wixely / Agent | 2026-09-28 |
...
Owner: Wixely / Agent. Next action: run the Windows Service lifecycle verifier on a disposable elevated host and record the result in [verification status](verification.md). Review: 2026-09-28.
