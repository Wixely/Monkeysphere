...
- Photos in a contact file are stored as record images. A photo held inside the card needs no network. A card that only links to a photo (as Google exports do) is fetched **only if you tick the box** for that import, and Monkeysphere tells you plainly that doing so contacts those servers and reveals your IP address, the time, and that you hold those contacts. It is the only outbound request the application ever makes.
