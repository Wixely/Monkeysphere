...
The authenticated map plots coordinate-bearing records through bounded, paged spatial queries. Record-type and location-field filters narrow the view, close pins cluster together, and selecting a pin or cluster reveals linked record summaries. Both coordinate editing and multi-record visualization use the vendored OpenLayers client with a local graticule and make no external tile or geocoding request.
