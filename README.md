...
MCP also exposes `get_instance_info` and `get_capabilities` for version, tool permissions, domain support, and effective transport limits. They accept the implemented read/write grants or the narrower `instance.read` scope; `instance.read` does not authorize record or domain queries. Remote access settings let the administrator select supported permissions when rotating a credential, preserving its existing grants by default. Paged domain/type/field/relationship discovery and field-type input schemas let clients inspect complete structures. Type reads include field configuration and choice options; record values retain the display text and add raw scalar and structured temporal data. An explicitly selected `records.write` grant permits validation, creation and revision-checked patching, with durable retry receipts. Clients can also preview and atomically apply batches of up to 100 creates/patches. Existing credentials do not gain writes automatically. A separate `records.delete` grant permits revision-checked deletion, with impact previews for dependent data and durable media cleanup. Relationship creation/removal uses an explicit `relationships.write` grant; record/relationship-type creation, field creation/attachment, preset installation and onboarding use `structure.write`. These commands check revisions and return durable retry receipts. Domain creation and rename use a separate `domains.manage` grant and durable registry receipts. Interrupted creation resumes the reserved identity on retry or startup. The read-only `query_records` tool combines typed filters and field/name sorting with strict paging bounds. A separate `contacts.import` grant permits bounded contact-file upload, validation, durable preview inspection and transactional application with paged retry outcomes. A further `contacts.export` grant permits reading explicitly selected Person records back out as one bounded vCard 4.0 document; importing and exporting are deliberately separate authorities and neither implies the other. Separate `media.write` and `media.read` grants transfer record images: an upload declares whether it is a contact file or an image, and bytes staged for one purpose can never be consumed as the other. Reading an original returns the retained bytes, which can still carry camera metadata the display copies remove, so it is gated apart from writing. Broader management remains planned. See the [current MCP contract](docs/mcp-contract.md) and [management implementation plan](docs/mcp-management-plan.md).
...
- Published alpha images are available from GitHub Container Registry for Linux x64: `docker pull ghcr.io/wixely/monkeysphere:alpha`. Use an immutable version tag such as `0.2.0-alpha.1` for repeatable deployments. Do not start a new deployment from the `0.1.0-alpha.1` or `0.1.0-alpha.2` ZIP downloads: their migration ledgers cannot be upgraded by any later version, and their release notes explain why.
- Contact imports can keep more of what the file carries. Categories, address, organisation, job title, family and given name, social handles and the Facebook ID that HTC phones bury inside a note are each offered during the import with a count of how many contacts they affect; tick one and Monkeysphere creates the field and fills it. A note containing an HTCData block is imported without it either way, so your notes stop filling up with markup. The same fields can be created and filled afterwards from the source data already retained, without the original file and without overwriting anything you have typed.
- Photos in a contact file are stored as record images. A photo held inside the card needs no network. A card that only links to a photo (as Google exports do) is never fetched until **you have read the addresses yourself**. The import lists every address it would contact, in full, grouped by host and labelled with the contact it belongs to, and you approve hosts individually after reading them. Monkeysphere then contacts only the addresses you approved, and tells you plainly that doing so reveals your IP address, the time, and that you hold those contacts. It is the only outbound request the application ever makes.
- One unusable contact does not cost you the rest of a bulk import. A vCard the parser cannot read — no formatted name, an unsupported version, a mangled property line — is listed with its position in the file, whatever identified it, and why, and every other contact imports normally. A file whose structure is broken outright, such as a missing END marker, is still refused whole, because there is no safe way to tell where one contact ends and the next begins.
- Imported contacts keep their raw source data, and you can look at it. Every record imported from a vCard gains an **Imported source data** panel showing where it came from and every line the file contained, including custom properties and embedded payloads that no field maps to. Lines marked *not used* exist only there. Large embedded values show a preview and a download for the whole thing. The same material is readable over MCP with `get_record_source` and `read_record_source_value`.
- Backstage is disabled by default. Set `MONKEYSPHERE_BACKSTAGE_AVAILABLE=true` in Docker (or `Monkeysphere__Backstage__Available=true` in .NET configuration) to expose **Settings → Backstage**, where an account can enter a 24-hour mode that reveals records marked hidden and adds the option to hide or unhide a record. Outside that mode a hidden record appears nowhere at all: not in search, the graph, the map, the calendar, reminders, contact export, or any relationship on a record that links to it. Turning the setting back off leaves already-hidden records hidden and unreachable by everyone, including remote credentials; the app warns at startup with the count and how to reach them again. Backstage conceals records from ordinary use of the app; it is not encryption and not protection from whoever administers the host, the volume, or the backups.
