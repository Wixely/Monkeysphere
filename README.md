...
- Published alpha images are available from GitHub Container Registry for Linux x64: `docker pull ghcr.io/wixely/monkeysphere:alpha`. Use an immutable version tag such as `0.1.0-alpha.5` for repeatable deployments. Do not start a new deployment from the `0.1.0-alpha.1` or `0.1.0-alpha.2` ZIP downloads: their migration ledgers cannot be upgraded by any later version, and their release notes explain why.
